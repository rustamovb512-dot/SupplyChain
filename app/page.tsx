      ...
      {steps.map((step, index) => (
        <div key={step.status} className="flex gap-3">
          <div className="flex flex-col items-center">
            {step.done ? (
              <CheckCircle2 className={cn("h-5 w-5", step.current ? "text-slate-900" : "text-green-600")} />
            ) : (
          ...
          <div className="pb-2 text-sm">
            <span className={step.current ? "font-medium text-slate-900" : "text-slate-500"}>
                ...
                >
                  ...
                  <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                    {onChangeStatus ? (
                      ...
                      <Btn variant="outline" className="text-xs" onClick={() => setSelected(o)}>
                        Подробно
                      </Btn>
                    )}
                  </td>
          ...
          <div className="space-y-3 p-5 text-sm">
            ...
            {a.role !== "super_admin" ? (
              <div className="flex flex-wrap gap-2">
                ...
                <Btn variant="danger" className="text-xs" onClick={() => onDelete(a.id)}>
                  <Trash2 className="mr-2 h-4 w-4" /> DELETE
                </Btn>
              </div>
            ) : (
              <div className="flex items-center gap-2 text-slate-500">
                <Shield className="h-4 w-4" /> Основной владелец системы
              </div>
            )}
          </div>
        </Panel>
...
function CreateOrderModal({
  ...
  onCreate: (order: Omit<Order, "id" | "cargo_number" | "order_number">) => Promise<void> | void;
}) {
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Создать новый заказ">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Цена для клиента">
            <TextInput value={clientPrice} onChange={(e) => setClientPrice(e.target.value)} />
          </Field>

          <Field label="Полученный аванс">
            <TextInput value={advanceAmount} onChange={(e) => setAdvanceAmount(e.target.value)} />
          </Field>

          <Field label="Аванс от">
            <TextInput value={advanceFrom} onChange={(e) => setAdvanceFrom(e.target.value)} />
          </Field>

          <Field label="Договор с водителем">
            <TextInput value={driverDeal} onChange={(e) => setDriverDeal(e.target.value)} />
          </Field>
...
export default function App() {
  ...
  async function updateOrderStatus(id: string, status: OrderStatus) {
    if (!supabase || !session) {
      setOrders((prev) =>
        prev.map((o) => (o.id === id ? { ...o, status, archived: status === "PAID" } : o)),
      );
      return;
    }
    ...
    const { error } = await supabase
      .from("orders")
      .update({ status, archived: status === "PAID" })
      .eq("id", id);
      ...
      <div className="grid min-h-screen lg:grid-cols-[280px_1fr]">
        <aside className="border-r border-slate-200 bg-white p-4 lg:p-6">
          ...
          <Panel className="bg-slate-50">
            <div className="space-y-3 p-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                {profile?.full_name || session?.user.email || "Demo user"}
              </div>
              <p className="text-slate-500">
                Текущая роль: <span className="font-medium text-slate-900">{role}</span>
              </p>
          ...
          <div className="mt-8 space-y-2">
            {[
              ...
              ...(role === "super_admin"
                ? [{ key: "admins", icon: Users, label: "Администраторы" }]
                : []),
            ].map((item) => (
              <button
                key={item.key}
                ...
                className={cn(
                  ...
                  activeTab === item.key ? "bg-slate-900 text-white" : "hover:bg-slate-100",
                )}
              >
          ...
          <div className="mt-10 space-y-3">
            {!usingDemo ? (
              <Btn variant="outline" className="w-full" onClick={() => void signOut()}>
        ...
        <main className="p-4 lg:p-8">
          <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2">
              <Btn variant="outline">
                <Globe className="mr-2 h-4 w-4" /> RU
              </Btn>
              {role === "admin" || role === "super_admin" ? (
                <CreateOrderModal onCreate={(order) => void createOrder(order)} />
              ) : null}
            </div>
          </div>
          ...
          {pageLoading ? (
            <div className="mb-6 flex items-center gap-2 rounded-2xl border border-slate-200 bg-white p-4 text-sm">
          ...
          {activeTab === "orders" ? (
            <div className="space-y-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="flex w-full max-w-4xl gap-3">
                  <div className="relative w-full max-w-lg">
                    ...
                    <TextInput
                      className="pl-9"
                      placeholder="Поиск по номеру, получателю, маршруту"
                      value={search}
                  ...
                  <div className="relative w-[260px]">
                    ...
                    <select
                      ...
                      className="w-full rounded-xl border border-slate-300 px-9 py-2 text-sm"
                    >
              ...
              <OrdersTable
                orders={activeOrders}
                role={role}
                onChangeStatus={(id, status) => void updateOrderStatus(id, status)}
              />
            </div>
          ) : null}
            ...
            <Panel>
              <div className="space-y-2 p-5 text-sm text-slate-600">
                <div className="flex items-center gap-2 text-base font-semibold text-slate-900">
                  <CircleHelp className="h-5 w-5" /> Как пользоваться
                </div>
          ...
          {activeTab === "admins" && role === "super_admin" ? (
            <AdminsGrid
              admins={admins}
              onStatusChange={(id, status) => void updateAdminStatus(id, status)}
